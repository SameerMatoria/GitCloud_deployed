...
export default function RepoClient() {
  ...
  const handleDelete = async (file) => {
    ...
    try {
      const res = await api.delete('/api/delete-file', {
        data: { repo, path: file.path, sha: file.sha },
      });
  ...
  const getRawUrl = (item) =>
    `https://raw.githubusercontent.com/${username}/${repo}/main/${item.path}`;

  const foldersAndOtherFiles = contents.filter(
    (item) => item.type === 'dir' || !item.name.match(/\.(jpg|jpeg|png|gif)$/i)
  );
      ...
      <div className="space-y-4 mb-8">
        {foldersAndOtherFiles
          .filter((item) => {
            ...
            const exclude = ['mp3', 'wav', 'ogg', 'mp4', 'mov', 'webm', 'jpg', 'jpeg', 'png', 'gif', 'bmp', 'svg', 'webp'];
